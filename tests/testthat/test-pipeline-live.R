# ==============================================================================
# LIVE Pipeline Tests for nvschooldata
# ==============================================================================
#
...
test_that("Nevada DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Nevada DOE URL and verify HTTP 200")
})
...
test_that("Can download Nevada enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Nevada enrollment file", {
  skip_if_offline()
...
test_that("nvschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- nvschooldata::get_available_years()
  ...
  tryCatch({
    raw <- nvschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- nvschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- nvschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- nvschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- nvschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- nvschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- nvschooldata:::get_cache_path(2024, "enrollment")
