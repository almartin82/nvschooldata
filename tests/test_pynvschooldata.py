...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = nv.get_available_years()
...
class TestFetchEnr:
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = nv.fetch_enr(years['max_year'])
        # Total enrollment in Nevada varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Nevada should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = nv.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = nv.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pynvschooldata as nv
        years = get_test_years()
        df_single = nv.fetch_enr(years['max_year'])
        df_multi = nv.fetch_enr_multi([years['max_year']])
